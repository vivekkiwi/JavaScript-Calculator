...
1. Inputs are taken from number keys for numbers and operators.
2. Enter key of numpad is used to get the result of the input value.
3. Backspace key can be used to erase a single letter.
4. For implementing parenthesis using keyboard just type { and } of keys which are above alphabet keys.
...
2. sin, cos, tan, log, sqrt, and approx works on single display input but not in use with expression form.
